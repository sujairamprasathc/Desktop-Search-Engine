...
DocumentIDManager::DocumentIDManager() {
  ...
  document_id_store.open(LOCAL_APP_DATA_DIRECTORY + "lastdocumentid",
  ...
  document_id_map.open(LOCAL_APP_DATA_DIRECTORY + "documentidmap",
...
unsigned int DocumentIDManager::getNumberOfDocuments() {
  ...
  document_id_store.read(reinterpret_cast<char*>(&document_id),
                         sizeof(document_id));

  return document_id;
}

unsigned int DocumentIDManager::getNewDocumentID(
  ...
  document_id_store.write(reinterpret_cast<char*>(&document_id),
                          sizeof(document_id));
