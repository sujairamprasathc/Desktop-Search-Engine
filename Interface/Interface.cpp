...
int main(int argc, char **argv) {
  if (argc < 2) {
    cout << "USAGE : " << argv[0]
         << " <word> [<boolean operator> <word> <boolean operator> <word> ...]"
         << endl;
    exit(-1);
  }
  ...
  for (int i = 2, j = 0; i < argc; i += 2, ++j) {
    if (strcmp(argv[i], "and") == 0) {
      ...
      set_intersection(search_results[j + 1].begin(),
                       search_results[j + 1].end(),
                       final_search_results.begin(), final_search_results.end(),
                       temp_result_storage.begin());
      final_search_results = temp_result_storage;
    } else if (strcmp(argv[i], "or") == 0) {
      list<unsigned int> temp_result_storage(final_search_results.size() +
                                             search_results[j + 1].size());
      set_union(search_results[j + 1].begin(), search_results[j + 1].end(),
                final_search_results.begin(), final_search_results.end(),
                temp_result_storage.begin());
      final_search_results = temp_result_storage;
    } else if (strcmp(argv[i], "stem") == 0) {
      list<unsigned int> temp_result_storage(final_search_results.size());
      set_difference(final_search_results.begin(), final_search_results.end(),
                     search_results[j + 1].begin(), search_results[j + 1].end(),
                     temp_result_storage.begin());
      final_search_results = temp_result_storage;
    }

    // Remove extra elements
    final_search_results.erase(
        find(final_search_results.begin(), final_search_results.end(), 0),
        final_search_results.end());
  }
